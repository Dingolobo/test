...
def channels_to_xmltv(channels):
    ...
    for cid, chdata in channels.items():
        for event in chdata['events']:
            ...
            # Poster del programa (thumbnail del event) - Múltiples icons para fallback
            thumbnail_raw = event.get('thumbnail', '')
            if thumbnail_raw:
                # Icon 1: Horizontal optimizado con 'h9' y ancho fijo (preferido)
                event_thumbnail_h9 = thumbnail_raw.replace('v9', 'h9') + '.jpg?w=600'
                poster_url_h9 = f"https://zap2it.tmsimg.com/assets/{event_thumbnail_h9}"
                
                # Icon 2: Original sin modificar (vertical u original, fallback)
                # Si raw ya tiene .jpg, no agregamos; sino, sí
                if thumbnail_raw.endswith('.jpg'):
                    original_path = thumbnail_raw
                else:
                    original_path = thumbnail_raw + '.jpg'
                poster_url_original = f"https://zap2it.tmsimg.com/assets/{original_path}"
                
                # Agregar icons solo si no son idénticos (evita duplicados)
                if poster_url_h9 != poster_url_original:
                    # Primero el horizontal (preferido)
                    icon_h9 = ET.SubElement(prog, 'icon', src=poster_url_h9)
                    # Segundo el original (fallback)
                    icon_original = ET.SubElement(prog, 'icon', src=poster_url_original)
                else:
                    # Si son iguales, solo uno
                    icon = ET.SubElement(prog, 'icon', src=poster_url_h9)
