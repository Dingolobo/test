...
CHANNEL_IDS = [208, 482, 487, 613, 885, 737, 746, 748, 765, 804, 805, 882, 981, 982, 606, 1446, 969, 815, 967, 822, 814, 736, 808, 807, 809, 762, 801, 764, 806, 704, 705]
...
async def extract_valid_uuid():
    ...
    async with async_playwright() as p:
        ...
        # Definimos un evento para detener la escucha cuando se obtiene UUID válido
        stop_listening = asyncio.Event()

        async def handle_response(response):
            nonlocal valid_uuid
            if stop_listening.is_set():
                return  # Ya tenemos UUID válido, ignorar más respuestas
            url = response.url
            if url.startswith(CHANNEL_URL_PREFIX):
                ...
                if match:
                    ...
                    if uuid == GENERIC_UUID:
                        ...
                        valid_uuid = uuid
                        stop_listening.set()  # Detenemos la escucha
...
# (las copias tal cual, solo agregué el parámetro uuid en fetch_channel_contents)
...
def main(uuid):
    ...
    logger.info(f"Usando UUID dinámico: {uuid}")

    session = requests.Session()
    session.headers.update(HEADERS_EPG)

    logger.info("=== TEST FETCH PARA CANAL 222 (debug) ===")
